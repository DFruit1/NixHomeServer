...
This copy is installed as the **critic and QA layer**, not the primary designer. Before using it, read `.agents/skills/frontend-design/SKILL.md` — it owns the workflow, the project anti-slop rules (which take priority over this skill's stylistic preferences), and the stop conditions.

- Always available for reviewing rendered frontend work: `critique`, `audit`, and the deterministic detector (`npx impeccable detect <target>` or the skill's detector entry point).
- `layout`: run only when information is correct but spacing, density, hierarchy or arrangement are poor.
- `distill`: run only when the page carries excessive, redundant or low-value UI.
