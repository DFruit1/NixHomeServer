...
require_fixed modules/caddy/default.nix 'reverse_proxy http://127.0.0.1:${toString vars.mailArchiveOauth2ProxyPort}' \
  "The private mail archive hostname must reverse proxy through its dedicated oauth2-proxy instance."
...
require_fixed modules/oauth2-proxy/default.nix 'scope = "openid profile email";' \
  "OAuth2 Proxy must keep the files session lean by avoiding the groups scope."
...
require_fixed modules/kanidm/default.nix 'systems.oauth2.mail-archive-web = {' \
  "Kanidm provisioning must create the mail archive web client."
require_fixed modules/kanidm/default.nix 'originUrl = "https://${vars.emailsDomain}/oauth2/callback";' \
  "Kanidm provisioning must register the mail archive callback URL."
require_fixed modules/kanidm/default.nix 'basicSecretFile = config.age.secrets.mailArchiveOauth2ProxyClientSecret.path;' \
  "Kanidm provisioning must use the dedicated mail oauth2-proxy client secret."
require_fixed modules/kanidm/default.nix 'scopeMaps."mail-archive-users" = [ "openid" "profile" "email" "groups" ];' \
  "Kanidm provisioning must scope mail archive access to mail-archive-users."
...
require_fixed modules/audiobookshelf/default.nix 'audiobookshelf-storage-migration-v1' \
  "Audiobookshelf must normalize legacy migrated storage paths before the service starts."
...
require_fixed modules/audiobookshelf/default.nix '.backupPath = $backupDir' \
  "Audiobookshelf must rewrite its persisted backup path into the canonical appdata root."
...
require_fixed modules/kavita/default.nix 'Enabled = true;' \
  "Kavita must enable OIDC in the declarative startup config."
...
require_fixed modules/paperless/default.nix 'paperless-permissions-bootstrap' \
  "Paperless must bootstrap its local Users group and permissions for OIDC users."
require_fixed modules/paperless/default.nix "view_uisettings" \
  "Paperless must grant dashboard UI settings access through the bootstrap permissions."
require_fixed modules/paperless/default.nix "view_savedview" \
  "Paperless must grant saved-view access through the bootstrap permissions."
...
require_fixed modules/copyparty/default.nix 'idp-logout = "/oauth2/sign_out?rd=/oauth2/start?rd=%2F";' \
...
require_fixed modules/copyparty/default.nix '[/my-files/' \
  "Copyparty must keep the personal workspace mounted under the /my-files URL space."
require_fixed modules/copyparty/default.nix '[/shared]' \
  "Copyparty must expose a single shared root at /shared."
require_fixed modules/copyparty/default.nix '[/shared/documents]' \
  "Copyparty must keep the document ingest path nested under /shared."
require_fixed modules/copyparty/default.nix '[/shared/photos]' \
  "Copyparty must keep the photo ingest path nested under /shared."
require_fixed modules/copyparty/default.nix '"html-head-s"' \
  "Copyparty must rewrite the splash-page entries so users see /my-files and /shared at the top level."
require_fixed modules/copyparty/default.nix 'unlistcr: true' \
  "Copyparty must hide nested personal and shared child mounts from the splash-page volume list."
require_fixed modules/copyparty/default.nix 'copyparty-layout-sync' \
  "Copyparty must build the simplified shared layout before the service starts."
...
require_fixed modules/mail-archive-ui/oauth2-proxy.nix '--http-address=127.0.0.1:${toString vars.mailArchiveOauth2ProxyPort}' \
  "The dedicated mail oauth2-proxy must listen on the configured loopback port."
require_fixed modules/mail-archive-ui/oauth2-proxy.nix '--redirect-url=https://${vars.emailsDomain}/oauth2/callback' \
  "The dedicated mail oauth2-proxy must use the private emails callback URL."
require_fixed modules/mail-archive-ui/oauth2-proxy.nix '--client-id=mail-archive-web' \
  "The dedicated mail oauth2-proxy must use the dedicated Kanidm client id."
require_fixed modules/mail-archive-ui/oauth2-proxy.nix '--allowed-group=mail-archive-users' \
  "The dedicated mail oauth2-proxy must reject users outside mail-archive-users."
require_fixed modules/mail-archive-ui/oauth2-proxy.nix '--upstream=http://127.0.0.1:${toString cfg.port}' \
  "The dedicated mail oauth2-proxy must forward authenticated requests to the local mail archive UI service."
...
forbid_match modules/oauth2-proxy/default.nix 'scope = "openid profile email groups"' \
  "OAuth2 Proxy must not keep the broad files scope that bloats session cookies."
forbid_match modules/oauth2-proxy/default.nix 'oidc-groups-claim' \
  "OAuth2 Proxy must not keep the groups claim in the files flow."
...
require_match documentation/networking-and-access.md 'emails\.<domain>' \
  "Networking guide must document emails.<domain> as a private endpoint."
