...
let
  ...
  mapDisk = idx: diskId: {
    label =
      if idx < builtins.length vars.monitoredDataDiskIds then
        "disk${toString (idx + 1)}"
      else
        (builtins.elemAt vars.coldStoragePools (idx - builtins.length vars.monitoredDataDiskIds)).name;
    device = "/dev/disk/by-id/${diskId}";
  };
  monitoredDiskIds = vars.monitoredStorageDiskIds;
  monitoredDisks = builtins.genList (idx: mapDisk idx (builtins.elemAt monitoredDiskIds idx)) (builtins.length monitoredDiskIds);
...
in
{
  ...
  storage = {
    ...
    coldStoragePoolNames = map (pool: pool.name) vars.coldStoragePools;
  };
  ...
  backup = {
    ...
    selectionFile = "/persist/appdata/.nixos-managed/system-state-backup-target/selected-device";
