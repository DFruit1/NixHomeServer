...
source "$script_dir/lib-runtime-health.sh"
...
usage() {
  cat <<'EOF'
Usage: scripts/runtime-readiness.sh [--profile manual|deploy|monitor] [--deep] [--format text|json] [--require-online-zpool]
...
need curl host systemctl findmnt jq smartctl journalctl zpool zfs runuser stat date readlink
...
run_backup_checks() {
  ...
  mounted_source="$(findmnt -rn -o SOURCE --target "$mount_point" 2>/dev/null || true)"
...
emit_text
emit_text "== SMART =="
mapfile -t smart_specs < <(runtime_health_snapshot_query '.storage.monitoredDisks[] | "\(.label)=\(.device)"' | jq -r '.')
