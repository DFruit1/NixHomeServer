...
if (($# != 2)); then
  echo "Usage: scripts/helpers/send-storage-health-alert.sh <report-json> <report-txt>" >&2
  exit 1
fi
...
severity="$(jq -r '.storage.overallSeverity' "$report_json")"
previous_digest="$(cat "$digest_file" 2>/dev/null || true)"
previous_severity="$(cat "$severity_file" 2>/dev/null || true)"
normalized_digest="$(
  jq -c '{
    overallSeverity: .storage.overallSeverity,
    alerts: .storage.alerts,
    degradedDisks: [.storage.disks[] | select(.severity != "OK") | {label, severity, detail}],
    unhealthyMounts: [.storage.mounts[] | select(.severity != "OK") | {target, severity, detail}],
    zfs: {
      statusSeverity: .storage.zfs.statusSeverity,
      datasetSeverity: .storage.zfs.datasetSeverity,
      statusSummary: .storage.zfs.statusSummary,
      datasetSummary: .storage.zfs.datasetSummary
    }
...
body="$(
  jq -r --arg reportTxt "$report_txt" '
    [
      "Host: " + .hostname,
      "Severity: " + .storage.overallSeverity,
      "ZFS pool: " + .storage.zfs.statusSummary,
      "Report: " + $reportTxt,
      "Top findings:"
    ]
    + ((.storage.alerts | .[0:3]) | map("- " + .))
    | join("\n")
  ' "$report_json"
)"
