...
if [[ -f "$script_dir/lib-repo.sh" ]]; then
  source "$script_dir/lib-repo.sh"
elif [[ -n "${BACKUP_TARGET_REPO_ROOT:-}" && -f "${BACKUP_TARGET_REPO_ROOT}/scripts/lib-repo.sh" ]]; then
  source "${BACKUP_TARGET_REPO_ROOT}/scripts/lib-repo.sh"
else
  echo "❌ Could not locate scripts/lib-repo.sh." >&2
  exit 1
fi
...
state_dir="${BACKUP_TARGET_STATE_DIR:-/persist/appdata/.nixos-managed/system-state-backup-target}"
...
usage() {
  cat <<'EOF'
Usage: scripts/backup-target.sh <list|status|select|mount|unmount|clear> [selector]
...
Examples:
  scripts/backup-target.sh list
  scripts/backup-target.sh status
  sudo scripts/backup-target.sh select /dev/disk/by-id/<usb-backup>-part1
  sudo scripts/backup-target.sh select
  sudo scripts/backup-target.sh mount
EOF
}
...
partition_inventory_json() {
  if [[ -z "$partition_inventory_cache" ]]; then
    partition_inventory_cache="$(
      lsblk -J -p -o PATH,TYPE,TRAN,FSTYPE,LABEL,UUID,SIZE,MODEL,SERIAL,MOUNTPOINTS \
        | jq '
          [
            .blockdevices[]
            | select(.type == "disk")
...
device_is_protected() {
  ...
  while IFS= read -r candidate; do
    ...
    if jq -e --arg disk_id "$disk_id" '
      .mainDisk == $disk_id
      or (.zfsDataPoolDiskIds | index($disk_id) != null)
      or (.coldStorageDiskIds | index($disk_id) != null)
...
mounted_fstype_or_empty() {
  findmnt -rn -o FSTYPE --target "$mount_point" 2>/dev/null || true
}
...
mount_selected_target() {
  ...
  if [[ -z "$selected_device" ]]; then
    echo "❌ No backup target is selected. Use scripts/backup-target.sh select first." >&2
    exit 1
  fi
