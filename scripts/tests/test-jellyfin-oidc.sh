...
jq -e '
  ...
  and (.libraryBootstrapScript | contains("wait_for_authenticated_library_api"))
  and (.libraryBootstrapScript | contains("Jellyfin authenticated library API is not ready yet"))
  and (.libraryBootstrapScript | contains("jq -e '\''type == \"array\"'\''"))
