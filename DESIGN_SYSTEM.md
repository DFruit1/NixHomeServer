...
| Search pages results 50 at a time with a Load more control, offers a Sort control (relevance / newest), a Clear filters action, and mirrors the active query and filters in the URL | search (`src/ui.html`) | Solr and the API already supported paging and sort; the UI previously showed only the first 50 hits with no way to reach the rest, restore a search, or reorder by date. URL state makes a search shareable and lets the browser's back/forward replay filter changes, while Clear filters removes the need to toggle every control back. The Source select shows each source's display name while still filtering on its stable id. | — |
