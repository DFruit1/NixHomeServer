...
| Every library-health comparison names one recommended online source with its setup state ("Configured", "No setup needed", "Not set up", "Coming soon") and keeps the remaining candidates in an "Alternative Sources" dialog of documentation and setup links | media-manager (`metadata-health-view.tsx`, `health-source-recommendation.ts`) | A user with a missing field wants one next step, not a list; ranking is by deployed provider status first, then field capability and media-kind domain, and the dialog still exposes the rest of the sources without a wall of repeated cards | — |
