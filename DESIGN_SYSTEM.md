...
| Library health headers show the library's media-kind symbol instead of a repeated subtitle, an oversized title that falls back to an italic filename, and a right-hand artwork band that is desaturated, dimmed and faded into the page background | media-manager (`metadata-health-view.tsx`) | The kind symbol repeats information already in the group heading, so the header spends its space on recognition instead: the filename fallback stays distinguishable from a real title, and the artwork identifies the album at a glance without competing with the Current/Proposed comparison it sits beside | — |
| Every library-health comparison names one recommended online source with its setup state ("Configured", "No setup needed", "Not set up", "Coming soon") and keeps the remaining candidates in an "Alternative Sources" dialog of documentation and setup links | media-manager (`metadata-health-view.tsx`, `health-source-recommendation.ts`) | A user with a missing field wants one next step, not a list; ranking is by deployed provider status first, then field capability and media-kind domain, and a field's own domain joins the candidates for the kinds that can hold it (a video's missing subtitle reaches subtitle providers, which outrank generic movie sources for that field), while the dialog still exposes the rest of the sources without a wall of repeated cards | — |
...
| Search results present one uniform card for every source: source badge, title, a highlighted body snippet, a type/owner/date line, then a compact `Label: value` metadata line | search (`src/ui.html`) | Indexed sources (mail, paperless, FreshRSS, browsertrix) and runtime-federated sources (Kiwix native Xapian) are indistinguishable to the user; body text and key metadata appear for all. Unknown metadata keys are title-cased so a new source needs no UI change. Snippet text is HTML-escaped by the API, which adds only the `<em>` highlight markers, so the client inserts it directly and a body containing a literal `<em>` cannot masquerade as a highlight. | — |
| Search pages results with a Load more control and a Per page choice (25/50/100), offers a Sort control (relevance / newest), a Clear filters action, and mirrors the active query and filters in the URL | search (`src/ui.html`) | Solr and the API already supported paging, sort, and a page size; the UI previously showed only the first 50 hits with no way to reach the rest, restore a search, reorder by date, or widen the page. URL state makes a search shareable and lets the browser's back/forward replay filter changes, while Clear filters removes the need to toggle every control back. The Source select shows each source's display name while still filtering on its stable id. | — |
...
Media health defaults to every visible library and automatically inspects all bounded API pages, with at most three libraries scanning concurrently. Results use effective titles, grouped field comparisons, labelled source alternatives, and collapsible file paths. Each result header pairs the library's media-kind symbol with an oversized title (italic when it falls back to a filename) over a faded artwork band, and each issue names one recommended online source plus its setup state, with the remaining candidates behind an "Alternative Sources" dialog. Audio results group by album (parent folder) into one article per album with per-file review links, so a 30-file audiobook never renders 30 separate warnings; grouped problems carry their affected files inline. Filename-derived guesses remain fallback metadata but do not create conflicts with actual metadata sources. On phones, current and proposed values stack rather than requiring horizontal scrolling. Scan failures preserve other libraries' results and mark totals as incomplete.
...
Library search is deliberately split. The library toolbar filters the currently loaded titles and filenames client-side and sends no query anywhere; full metadata and full-text discovery stays in the Search app and is reached through an optional "Advanced search" link that renders only when the deployment registers the `search` capability. Library reads are always served from the catalog cache and never walk the filesystem: the toolbar "Refresh" runs one cooldown-limited scan of the active category's roots, and a background scanner reconciles due roots on an adaptive backoff (15 then 30, 45, and 60 minutes, then hourly to a 24-hour ceiling, resetting on change). The Activity view lists the durable mutation queue as rows with a state badge and owner-only Retry/Cancel actions, and states read-only mode inline.
