...
use self::{
    checks::{
        ...
        parse_group_words, removed_runtime_groups, sftp_authorized_keys_path, sftp_chroot_path,
        shared_mount_path, usb_mount_path, user_root_path, RemovedRuntimeGroup, RemovedRuntimeKind,
    },
    output::{merge_warnings, render_readiness_human, render_readiness_lines, sftp_next_actions},
...
use crate::{
    ...
    ops::local_runtime::{
        command_probe_payload, local_command_check, run_root_action, status_from_success,
        CheckStatus, ConvergencePolicy, LocalCommandSpec, RootAction, RuntimeCheck,
        RuntimeCheckResult, RuntimeReport,
    },
...
use super::user::{human_user_summary, load_user};
...
pub fn test_sftp_login(
    ...
    test_sftp_login_with_policy(cli, config, account_id, ConvergencePolicy::default())
}
...
pub fn test_sftp_login_with_policy(
    ...
    let warnings = person.warnings.clone();
...
fn build_runtime_checks(
    ...
    if sftp_member || scope == RuntimeScope::SftpLogin {
        ...
        checks.push(file_nonempty_check(
            cli.clone(),
            "ssh.authorized_keys.user_key_present",
            "user SFTP public key is installed",
            true,
            sftp_authorized_keys_path(config, account_id),
        ));
    } else {
        checks.push(skipped_check(
            "filesystem.chroot.exists",
...
fn file_nonempty_check(
    ...
    if !required {
        return skipped_check(id, label, "skipped because this file is not expected");
    }
    let path_string = path.display().to_string();
    let spec = LocalCommandSpec::new("test", ["-s".to_string(), path_string.clone()]);
    local_command_check(
        cli,
        id,
        label,
        true,
        format!("local test -s {path_string}"),
        spec,
        move |execution| {
            let success = execution.result.allowed_success(&BTreeSet::from([0]));
            let summary = if success {
                format!("Public key file exists and is non-empty: {path_string}")
            } else {
                format!("Public key file is missing or empty: {path_string}")
            };
            RuntimeCheckResult {
                status: status_from_success(success),
                summary,
...
mod tests {
    ...
    fn diagnostics_use_renamed_sftp_runtime_values() {
        ...
        fs::write(
            authorized_keys_dir.join("alice"),
            "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAITestKey alice\n",
        )
...
"#,
        ...
        let config = SftpRuntimeConfig {
            ...
            user_sftp_authorized_keys_dir: authorized_keys_dir.display().to_string(),
        ...
        assert_eq!(
            output.details["sftp_runtime"]["userSftpAuthorizedKeysDir"],
            authorized_keys_dir.display().to_string()
        );
        let rendered = serde_json::to_string(&output.details).expect("details json");
        assert!(rendered.contains("ssh.authorized_keys.user_key_present"));
