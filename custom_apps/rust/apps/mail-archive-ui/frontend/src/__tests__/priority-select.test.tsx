...
import { priorityFailureMessage, setPriorityClass, submitPriorityChange } from '../shared/dom';
...
describe('priority select island helpers', () => {
  ...
  it('submits expected form data and redirects on success', async () => {
    ...
    const fetchMock = vi.fn(async (_url: string, init: RequestInit) => {
      ...
      return new Response(JSON.stringify({ ok: true, return_to: '/search?q=invoice' }), { status: 200 });
    });
  ...
  it('restores previous value and class on failure', async () => {
    ...
    select.value = 'low';
    const result = await submitPriorityChange(select, {
      fetch: vi.fn(async () => new Response(JSON.stringify({ ok: false, message: 'denied' }), { status: 403 })) as unknown as typeof fetch,
      assign: vi.fn(),
      currentPath: () => '/search',
    });
  ...
  it('renders a useful failure message', () => {
    expect(priorityFailureMessage('denied')).toContain('Priority change failed.');
    expect(priorityFailureMessage('denied')).toContain('Server response: denied');
  });
});
