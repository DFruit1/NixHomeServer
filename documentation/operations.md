...
minutes), then postpones it while a critical task is active, waiting up to a
grace window (default 60 minutes). A task that outlasts the grace window is
treated as hung and the shutdown proceeds. It is covered by the host's existing
NOPASSWD admin contract, so no sudo password is prompted.

Critical tasks are the bash arrays `CRITICAL_UNITS`, `CRITICAL_PROCESSES`, and
`CRITICAL_COMMANDS` in `/etc/nixhomeserver/shutdown-guard.conf`, generated by
`modules/Core_Modules/base-system/shutdown-guard.nix`. Only task units belong
in `CRITICAL_UNITS`; always-on daemons such as `kopia.service`,
`media-manager.service`, and `youtube-downloader.service` stay active by design
and must never block a shutdown. Edit the module, not the host, then redeploy.

Server commands (run over SSH as the admin user):
...
broadcasts a local poweroff after `--local-grace` minutes (default 5). Cancel
the local poweroff with Ctrl-C in that window or `loginctl -c`.

```bash
scripts/admin/desktop-server-shutdown.sh --timeout 60 --grace 60 --local-grace 5
```

Use `--dry-run` to print the plan without touching the server or the PC, and
`--no-local-shutdown` to manage only the server.
