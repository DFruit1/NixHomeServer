...
Critical tasks are the bash arrays `CRITICAL_UNITS` and `CRITICAL_PROCESSES` in
`/etc/nixhomeserver/shutdown-guard.conf`, generated by
`modules/Core_Modules/base-system/shutdown-guard.nix`. Edit the module, not the
host, then redeploy.
