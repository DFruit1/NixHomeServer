  ...
  in
    {
      ...
      config = {
        ...
        sambaDataPath = sambaSettings.data.path;
        sambaDataValidUsers = sambaSettings.data."valid users";
        sambaDataAdminUsers = sambaSettings.data."admin users";
...
require_json_equal "$(snapshot_query '.config.sambaDataPath')" "$(snapshot_query '.vars.dataRoot')" \
  "The admin SMB share must expose the full data root."
require_json_equal "$(snapshot_query '.config.sambaDataValidUsers')" "\"$(snapshot_query '.vars.kanidmAdminUser' | jq -r .)\"" \
  "The admin SMB share must be gated to the delegated admin user."
require_json_equal "$(snapshot_query '.config.sambaDataAdminUsers')" "\"$(snapshot_query '.vars.kanidmAdminUser' | jq -r .)\"" \
...
require_json_equal "$(snapshot_query '.config.mailArchivePaperlessConsumeRoot')" '"/mnt/data/media/documents/inbox/mail-archive"' \
  "Mail archive UI must point Paperless filing at the dedicated consume subtree."
require_json_equal "$(snapshot_query '.config.mailArchivePaperlessStagingDir')" '"/mnt/data/media/documents/.mail-archive-paperless-staging"' \
...
require_json_equal "$(snapshot_query '.config.paperlessExporterDirectory')" '"/mnt/data/media/documents/export"' \
...
require_json_contains "$(snapshot_query '.config.mailArchiveUiReadWritePaths')" "/mnt/data/media/documents/inbox/mail-archive" \
  "Mail archive UI must only gain write access to the dedicated Paperless consume subtree."
require_json_contains "$(snapshot_query '.config.mailArchiveUiReadWritePaths')" "/mnt/data/media/documents/.mail-archive-paperless-staging" \
