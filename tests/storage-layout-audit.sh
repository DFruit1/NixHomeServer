...
require_match <(printf '%s\n' "$clean_output") '^✅ retired root empty: /mnt/data/shared/documents$' \
  "The storage layout audit must report empty retired shared-document roots."
require_match <(printf '%s\n' "$clean_output") '^✅ retired root empty: /mnt/data/shared/photos$' \
  "The storage layout audit must report empty retired shared-photo roots."
...
printf 'stale\n' >"$tmpdir/mnt/data/media/leftover.txt"
...
require_match <(printf '%s\n' "$dirty_output") '^❌ retired root non-empty: /mnt/data/media$' \
